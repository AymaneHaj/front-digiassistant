...
export default function ChatPage() {
  ...
  useEffect(() => {
    // Only start if: authenticated, no conversation, not already initializing, no history, and haven't initialized yet
    if (isAuthenticated && !conversationId && !isInitializing && history.length === 0 && !isLoading && !hasInitialized.current) {
      console.log('[DiagnosticPage] Starting chat initialization...');
      hasInitialized.current = true; // Mark as initialized
      dispatch(resumeOrStartChat());
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [dispatch, isAuthenticated, conversationId, isInitializing]); // Only depend on stable values
  ...
  return (
    <div className="h-[calc(100vh-56px)] sm:h-[calc(100vh-64px)] md:h-[calc(100vh-100px)] lg:h-[calc(100vh-120px)] flex flex-col">
      <ChatWindow
        history={history}
        isLoading={isLoading}
