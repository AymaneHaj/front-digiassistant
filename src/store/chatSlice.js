    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(resumeOrStartChat.fulfilled, (state, action) => {
                ...
                if (action.payload.type === 'resume') {
                    ...
                    state.history = state.history.filter(msg => {
                        ...
                        if (seen.has(key)) {
                            console.log('[chatSlice] Removing duplicate message:', msg.content.substring(0, 50));
                            return false;
                        }
                        seen.add(key);
                        return true;
            ...
            .addCase(sendMessage.fulfilled, (state, action) => {
                ...
                if (ai_question) {
                    // Check if this exact question already exists in history (check last few messages to avoid duplicates)
                    const recentMessages = state.history.slice(-5); // Check last 5 messages
                    const questionExists = recentMessages.some(
                    ...
                    if (!questionExists) {
                        ...
                        console.log('[chatSlice] Question already exists in recent history, skipping duplicate');
                    }
                } else {
